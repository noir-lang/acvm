    ...
    (
        ...
        proptest! {
            ...
            fn $rol(x in 0..$u::MAX, y in 0..32_u32) {
                ...
                let mut acvm = ACVM::new(&StubbedBackend, extra_gates, witness_assignments, BTreeMap::new());
            ...
            fn $add(x in 0..$u::MAX, y in 0..$u::MAX, z in 0..$u::MAX) {
                ...
                let mut acvm = ACVM::new(&StubbedBackend, gates, witness_assignments, BTreeMap::new());
                let solver_status = acvm.solve();
    ...
    (
        ...
        proptest! {
            ...
            fn $name(input_values in proptest::collection::vec(0..u8::MAX, 1..50)) {
                ...
                // solve witnesses
                let mut acvm = ACVM::new(&StubbedBackend, circuit.opcodes, witness_assignments.into(), BTreeMap::new());
                let solver_status = acvm.solve();
