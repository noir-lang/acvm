...
pub enum BackendError {
    #[error("could not deserialize proving key")]
    InvalidProvingKey(Vec<u8>),
    #[error("could not deserialize verification key")]
    InvalidVerificationKey(Vec<u8>),
    #[error("encountered malformed opcode")]
    MalformedOpcode(Opcode),
    #[error("called with unexpected opcode")]
    UnexpectedOpcode(Opcode),

    /// A catch-all error variant for errors which are specific to the backend in question.
    #[error(transparent)]
...
pub trait SmartContract {
    ...
    fn eth_contract_from_vk(&self, verification_key: &[u8]) -> Result<String, BackendError>;
}
