...
#[derive(Debug, Error)]
#[error(transparent)]
struct BackendError(#[from] Error);
...
impl Default for BarretenbergSolver {
    fn default() -> BarretenbergSolver {
        BarretenbergSolver::new()
    }
}
...
#[cfg(feature = "native")]
mod native {
    use super::{BarretenbergSolver, Error, FeatureError, FieldElement};

    impl BarretenbergSolver {
        pub(crate) fn new() -> BarretenbergSolver {
            BarretenbergSolver {}
        }
    }

    pub(super) fn field_to_array(f: &FieldElement) -> Result<[u8; 32], Error> {
        let v = f.to_be_bytes();
        let result: [u8; 32] =
...
mod wasm {
    ...
    use super::{BarretenbergSolver, Error, FeatureError};
    ...
    impl BarretenbergSolver {
        ...
        pub(crate) fn new() -> BarretenbergSolver {
            ...
            BarretenbergSolver { memory, instance, store: RefCell::new(store) }
        }

        #[cfg(target_arch = "wasm32")]
        pub(crate) async fn initialize() -> BarretenbergSolver {
